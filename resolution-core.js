...
export function inferOrthogonalInPlaneHKL(rl, sv1, sv2){
  ...
  const uu=dot(qU,qU);
  const qVperp=qV.map((x,i)=>x-dot(qV,qU)/uu*qU[i]);
...
export function inferOutOfPlaneHKL(rl, sv1, sv2){
  ...
  // Keep the U x V sign so W is consistent with the right-handed
  // sample frame constructed in UB_calc.
  const qW=normalize(qWraw);
...
export function UB_calc(lc,rl){
  ...
  let U2=normalize(q2perp);
  const U3=normalize(cross(U1,U2));
  U2=normalize(cross(U3,U1));
  const U=[U1,U2,U3];
  const B=[
    [rl.astar[0],rl.bstar[0],rl.cstar[0]],
    [rl.astar[1],rl.bstar[1],rl.cstar[1]],
...
export function calcResolution(lc,rl,col,mos,config,approximation,focusing,geom,calc,unitMode='rlu'){
  ...
  if(Math.abs(qOut)>1e-8*Math.max(qvNorm,1)) throw new Error('The requested Q is outside the scattering plane defined by sv1 and sv2.');
  ...
  const widths={
    ...
    Ucoh:coherentFwhm(RM_U,0), Vcoh:coherentFwhm(RM_V,0), Wcoh:coherentFwhm(RM_W,0), Ecoh:coherentFwhm(RM_U,2)
  };
  ...
  const projUE=ellipsePoints(schur2(RM_U,[0,2]),normU,1), sliceUE=ellipsePoints(submatrix(RM_U,[0,2],[0,2]),normU,1);
  const projVE=ellipsePoints(schur2(RM_V,[0,2]),normV,1), sliceVE=ellipsePoints(submatrix(RM_V,[0,2],[0,2]),normV,1);
  const projWE=ellipsePoints(schur2(RM_W,[0,2]),normW,1), sliceWE=ellipsePoints(submatrix(RM_W,[0,2],[0,2]),normW,1);
