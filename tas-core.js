...
export function UB_calc(lc, rl) {
  ...
  const normal=cross(qu,qv);
  if (norm(normal)<EPS) throw new Error("sv1 and sv2 must not be parallel.");

  // Preserve the entered U/V handedness exactly.  No canonical front/back
  // normal is imposed: swapping U and V reverses the plane normal.
...
export function makeSpiceScatteringPlaneBasis(rl,uHkl,vHkl) {
  ...
  // Preserve the entered U/V handedness; ey therefore remains on the entered
  // V side after Gram-Schmidt projection.
